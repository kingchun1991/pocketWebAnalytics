;(function() {
  ...
  window.pocketWebAnalytics.get_data = function(vars) {
      ...
      var data = {
          ...
          r: (vars.referrer === undefined ? window.pocketWebAnalytics.referrer : vars.referrer),
          t: (vars.title === undefined ? window.pocketWebAnalytics.title : vars.title),
  ...
  window.pocketWebAnalytics.filter = function() {
      ...
      if (location.hostname.match(/(localhost$|^127\.|^10\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.|^192\.168\.|^0\.0\.0\.0$)/)) return 'localhost';
      if (location.protocol === 'file:') return 'localfile';
      if (localStorage && localStorage.getItem('skippwa') === 't') return 'disabled with #toggle-pocketwebanalytics';
      return false;
  };
      ...
      var send = function(elem) {
          return function() {
              window.pocketWebAnalytics.count({
                  event: true,
                  path: (elem.dataset.pocketwebanalyticsClick || elem.name || elem.id || ''),
                  title: (elem.dataset.pocketwebanalyticsTitle || elem.title || (elem.innerHTML || '').substr(0, 200) || ''),
                  referrer: (elem.dataset.pocketwebanalyticsReferrer || elem.dataset.pocketwebanalyticsReferral || ''),
              });
          };
      };

      Array.prototype.slice.call(document.querySelectorAll("*[data-pocketwebanalytics-click]")).forEach(function(elem) {
  ...
  if (location.hash === '#toggle-pocketwebanalytics') {
      if (localStorage.getItem('skippwa') === 't') {
          ...
          alert('PocketWebAnalytics tracking is now DISABLED in this browser until ' + location + ' is loaded again.');
      }
  }
